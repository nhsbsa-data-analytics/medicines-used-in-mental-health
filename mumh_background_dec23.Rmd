...
<small><sup>1</sup> Before March 2020 the NHSBSA maintained two drug databases that were difficult to align and reconcile. These were combined and the legacy system decommissioned by the ‘One Drug Database’ project.</small>
...
The NHSBSA does not capture information relating to a patient’s sex or gender from a prescription during processing activities. Gender is instead obtained from the PDS. Therefore, gender information is only available for patients that we have been able obtain a matched NHS number for.
...
The number of items for each BNF section in the 43 month period March 2020 to December 2023 were compared to the number of items expected to be prescribed based on trends from the ‘pre-COVID-19’ period. The monthly data for April 2015 to February 2020 was used to create a linear regression forecast for March 2020 to December 2023, with the actual totals compared. By using a linear regression, this predicts a future value based on a line of best fit of previous values. This linear regression extrapolated figures according to:
...
### 3.1 Additional quarterly breakdowns {.toc-ignore}

From the Q3 2023/24 quarterly release, we have changed which edition of the National Statistics Postcode Lookup (NSPL) we use, from the May 2023 NSPL to the November 2023 NSPL. More information on how we use the NSPL to assign patients to an IMD quintile can be found in section 2.1.3: 'Patient deprivation' in this document.

From the Q2 2023/24 quarterly release, we have added new measures to the summary tables for each BNF section. The average number of items per identified patient and the average Net Ingredient Cost (NIC) per patient have been added as additional columns into the breakdowns by month and chemical substance. These averages are the mean. A new table of the number of identified patients per 1,000 population by age band and gender has also been added.

From the Q1 2023/24 quarterly release, we have split the publication into smaller summaries,one for each BNF Section and one for the modelling of future prescribing. 

From 2022/23 we have amended the annual publication to combine it with the quarterly release for January to March 2023. Demographic information such as breakdowns by gender, age band, and IMD have been added into the quarterly summary tables to align it with the information reported in previous annual publications. These breakdowns have not been applied to the monthly tables.

From 1 July 2022, Sustainability and Transformation Partnerships (STPs) have been replaced by Integrated Care Boards (ICBs) within the NHS geographical structure. A breakdown of the data by ICB has been added to the annual and quarterly tables, replacing the breakdown by STP available in previous annual publications.

Adding these demographic and geographical breakdowns to the quarterly tables will allow users more timely access to data than when previously limited to the annual release.
...
The R code used to produce the newest release of the MUMH publication is also available from the [NHSBSA GitHub](https://github.com/nhsbsa-data-analytics).
...
A dispensing contractor or dispenser can be a community pharmacy or appliance contractor (a dispenser that specialises in dispensing dressings, appliances, and medical devices). 
