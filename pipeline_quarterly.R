...
#figure 3 quarterly antidepressant items and patients
figure_3a_data <- quarterly_0403$monthly_section |>
  #added financial year filter now only using rolling years
  dplyr::filter(`Financial Year` %!in% c("2015/2016", "2016/2017")) |>
  dplyr::group_by(`Financial Year`,
                  `Year Month`,
...
#figure 4 quarterly antidepressant age and gender
figure_4_data <- age_gender_extract_quarter |>
  dplyr::select(`Financial Quarter`,
                `BNF Section Name`,
                `BNF Section Code`,
                `Age Band`,
                `Patient Gender`,
                ...
                `Total Net Ingredient Cost (GBP)`) |>
  dplyr::filter(`BNF Section Code` == "0403") |>
    dplyr::filter(`Financial Quarter` == max(`Financial Quarter`)) |>
    dplyr::mutate(`Total Identified Patients` = signif(`Total Identified Patients`, 3))
...
rmarkdown::render("mumh-quarterly-narrative-0403a.Rmd",
                  output_format = "html_document",
                  output_file = "mumh-quarterly-narrative-0403a.html")
rmarkdown::render("mumh-quarterly-narrative-0403c.Rmd",
                  output_format = "html_document",
                  output_file = "mumh-quarterly-narrative-0403c.html")
